...
public class ExtendedPropertiesUtils {
    ...
    public static final String PARANOID_PROPERTIES = "/system/etc/aicp/" + PARANOID_MAINCONF;
    ...
    public static void setAppConfiguration(ParanoidAppInfo info) {
        if (sIsHybridModeEnabled) {
            ...
            int defaultDpi = getActualProperty(PARANOID_PREFIX + (isSystemApp ?
                    "system_default_dpi" : (info.path.length() == 0 ? "0" : "user_default_dpi")));
            int defaultLayout = getActualProperty(PARANOID_PREFIX + "user_default_layout");
